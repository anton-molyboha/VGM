...
__all__ = ['LinearSystemHtdTotFixedDTbalanceNRBC']
log = vgm.LogDispatcher.create_logger(__name__)

#------------------------------------------------------------------------------
#------------------------------------------------------------------------------

class LinearSystemHtdTotFixedDTbalanceNRBC(object):
...
#                      np.array(G.es['effResistance'])
                ...
                for vI in vertices:
                    ...
                    #it is now a double connecting Vertex
                        ...
                        if G.vs[vI]['rBC'] != None:
                            ...
                            print(inE)
                            print(outE)
                            print(G.neighbors(vI))
                            print(G.vs[G.neighbors(vI)]['pressure'])
                            G.vs[vI]['av'] = 1
                            if G.vs[vI]['degree']>1:
    ...
    #-------------------------------------------------------------------------------------------
         ...
         #------------------------------------------------------------------------------------------
            ...
            #Check if RBCs are lost
            if nRBCSumBefore != nRBCSumAfter:
                print('WARNING RBC CONSERVATION')
                print(nRBCSumBefore)
                print(nRBCSumAfter)
                print('vertex')
                print(vi)
                ...
                if nRBCSumBefore != nRBCSumAfter:
                    ...
                    if G.vs[vi]['vType'] == 2:
                        ...
                        if boolHttEdge == 1 or boolHttEdge2==1 or boolHttEdge3==1:
                            if boolHttEdge == 1:
                                if nRBCSumAfter - len(rRBC) != nRBCSumBefore:
                                    print('BIGERROR RBC CONSERVATION at inlet 1')
                            if boolHttEdge2 == 1:
                                if nRBCSumAfter - len(rRBC2) != nRBCSumBefore:
                                    print('BIGERROR RBC CONSERVATION at inlet 2')
                            if boolHttEdge3 == 1:
                                if nRBCSumAfter - len(rRBC3) != nRBCSumBefore:
                                    print('BIGERROR RBC CONSERVATION at inlet 3')
                        else:
        ...
        self._nRBCInList.append(self._nRBCIn)
        self._nRBCOutList.append(self._nRBCOut)
        if self._capDil != 0:
        ...
        nRBC=deepcopy(G.es['nRBC'])
        G.es['nRBC'] = [len(e['rRBC']) for e in G.es]
        G.es['nRBCChange']=np.array(G.es['nRBC'])-np.array(nRBC)
        if self._analyzeBifEvents:
            ...
            if SampleDetailed:
                ...
                if doSampling and tSample >= sStart and tSample <= sStop:
                    ...
                    if t > BackUpTStart:
                        ...
                        G['nRBCListIn']=self._nRBCInList
                        G['nRBCListOut']=self._nRBCOutList
