...
__all__ = ['LinearSystemHtdTotFixedDTreducedMinDist']
...
class LinearSystemHtdTotFixedDTreducedMinDist(object):
	...
	if init:
        ...
        if self._innerDiam:
            G.es['minDist'] = [0.5*vrbc / (np.pi * e['diameter']**2 / 4) for e in G.es]
        else:
            eslThickness = self._P.esl_thickness
            G.es['minDist'] = [0.5*vrbc / (np.pi * (d-2*eslThickness(d))**2 / 4) for d in G.es['diameter']]
        G.es['nMax'] = [np.floor(e['length']/ (2*e['minDist'])) for e in G.es] 
                    ...
                    if e2['httBC'] is not None:
                        ...
                        if len(e2['rRBC']) > 0:
                            ...
                            if e2['v_last'] == 0:
                                print('CHECK HERE')
                                print(e2.index)
                                print(e2['posFirst_last'])
                                print(e2['keep_rbcs'])
                                if len(e2['keep_rbcs']) > 0:
                                    if e2['keep_rbcs'] > e2['posFirstLast']:
                                        print('BIGWARNING 2')
                        ...
                        if e3['httBC'] is not None:
                            ...
                            if len(e3['rRBC']) > 0:
                                ...
                                if e3['v_last'] == 0:
                                    print('CHECK HERE')
                                    print(e3.index)
                                    print(e3['posFirst_last'])
                                    print(e3['keep_rbcs'])
                                    if len(e3['keep_rbcs']) > 0:
                                        if e3['keep_rbcs'] > e3['posFirstLast']:
                                            print('BIGWARNING 3')
