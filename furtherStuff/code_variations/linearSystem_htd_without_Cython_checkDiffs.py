...
__all__ = ['LinearSystemHtdWCythDiffs']
...
class LinearSystemHtdWCythDiffs(object):
	...
	self._init=init
        ...
        self._gen0_absFlowChange=[]
        self._gen1_absFlowChange=[]
        self._gen2_absFlowChange=[]
        self._gen3_absFlowChange=[]
        self._gen0_relFlowChange=[]
        self._gen1_relFlowChange=[]
        self._gen2_relFlowChange=[]
        self._gen3_relFlowChange=[]
        self._gen0_absVeloChange=[]
        self._gen1_absVeloChange=[]
        self._gen2_absVeloChange=[]
        self._gen3_absVeloChange=[]
        self._gen0_relVeloChange=[]
        self._gen1_relVeloChange=[]
        self._gen2_relVeloChange=[]
        self._gen3_relVeloChange=[]
        self._t=0
...
#                           if len(e['rRBC']) > 0 else (1e25, e.index) for e in G.es])
        ...
        #print(np.min(tlist.values()))
        dtmin = max(0.0, dtmin)
   
        #TODO: is that needed
	#Check if network is empty of RBC or if an unrealistic high dt results due to 
	#very low flow velocities, if though assign timestep
#	dtmin2=[]
#	dtminEdgeIndex2=[]
#	nRBCSum = np.sum(G.es[e]['nRBC'])
#        for vi in G['av']:
#            for ei in G.adjacent(vi):
#                dtmin2.append(G.es[ei]['length']/G.es[ei]['v'])
#                dtminEdgeIndex2.append(ei)
#
#	if nRBCSum == 0:
#	    print('Network is empty')
#            for e in range(len(dtmin2)):
#                if dtmin2[e] == min(dtmin2):
#                    dtmin=0.9*dtmin2[e]/2
#		    dtminEdgeIndex=dtminEdgeIndex2[e]
#		    break
#	if dtmin > min(dtmin2):
#            print('Limit dt to inflow')
#            for e in range(len(dtmin2)):
#                if dtmin2[e] == min(dtmin2):
#                    dtmin=0.9*dtmin2[e]
#                    dtminEdgeIndex=dtminEdgeIndex2[e]
#                    break
    ...
    def _update_flow_and_velocity(self):
        ...
        if self._t > 10000.0:
            absFlowChange=np.array(G.es['flow'])-np.array(flowOld)
            relFlowChange=absFlowChange/np.array(G.es['flow'])
        ...
        if self._t > 10000.0:
            absVeloChange=np.array(G.es['v'])-np.array(vOld)
            relVeloChange=absVeloChange/np.array(G.es['v'])
        if self._t > 10000.0:
            #Group Edges in dfferent generations
            gen1_Edges=[]
            gen2_Edges=[]
            rem_Edges=[]
            Gen0Vertex=[]
            gen0_Edges=[self._eiIn]
            edge=G.es[self._eiIn]
            splitVertex1=edge.source
            splitVertex2=edge.target
            #Edges belonging to dilated Edge
            #dummy=G.adjacent(splitVertex1)
            #for i in range(2):
            #    if dummy[i] not in gen0_Edges:
            #        gen1_Edges.append(dummy[i])
            Gen0Vertex.append(splitVertex1)
            #dummy=G.adjacent(splitVertex2)
            #for i in range(2):
            #    if dummy[i] not in gen0_Edges:
            #        gen1_Edges.append(dummy[i])
            Gen0Vertex.append(splitVertex2)

            #Vertices 'around' neighboring Edge
            Gen1Vertex=[]
            dummy2=G.neighbors(splitVertex1)
            for j in dummy2:
                if j in Gen0Vertex:
                    ...
                    Gen1Vertex.append(j)
            dummy2=G.neighbors(splitVertex2)
            for j in dummy2:
                if j in Gen0Vertex:
                    #print('already in List')
                    pass
            ...
            for j in Gen1Vertex:
                ...
                for i in range(len(dummy)):
                    if dummy[i] not in gen1_Edges:
                        if dummy[i] not in gen0_Edges:
                            gen1_Edges.append(dummy[i])
                            helpToGetGen2_Edges.append(dummy[i])
       
            gen1_Edges=np.unique(gen1_Edges)
       
            Gen2Vertex=[]
            for i in helpToGetGen2_Edges:
                eDummy=G.es[i]
                tuple=eDummy.tuple
                for j in tuple:
                    if j in Gen1Vertex:
                        #print('Is already a Generation 1 Vertex')
                        pass
                    else:
                        Gen2Vertex.append(j)
            ...
            for j in Gen2Vertex:
                dummy=G.adjacent(j)
                for i in range(len(dummy)):
                    if dummy[i] in gen1_Edges:
                        #print('Is already a Generation 1 Edge')
                        pass
                    else:
                        gen2_Edges.append(dummy[i])
                        helpToGetGen3_Edges.append(dummy[i])
       
            Gen3Vertex=[]
            for i in helpToGetGen3_Edges:
                eDummy=G.es[i]
                tuple=eDummy.tuple
                for j in tuple:
                    if j in Gen2Vertex:
                        #print('Is already a Generation 2 Vertex')
                        pass
                    else:
                        Gen3Vertex.append(j)
       
            for i in range(G.ecount()):
                if i not in gen1_Edges:
                    if i not in gen2_Edges:
                        rem_Edges.append(i)
                else:
                    #print('Edges is in Generation 1 or Generation 2')
                    pass

        if self._t > 10000.0:
            self._gen0_absFlowChange.append(absFlowChange[gen0_Edges[0]])
            self._gen0_relFlowChange.append(relFlowChange[gen0_Edges[0]])
            self._gen0_absVeloChange.append(absVeloChange[gen0_Edges[0]])
            self._gen0_relVeloChange.append(relVeloChange[gen0_Edges[0]])
    
            for i in range(len(gen1_Edges)):
                 self._gen1_absFlowChange.append(absFlowChange[gen1_Edges[i]])
                 self._gen1_relFlowChange.append(relFlowChange[gen1_Edges[i]])
                 self._gen1_absVeloChange.append(absVeloChange[gen1_Edges[i]])
                 self._gen1_relVeloChange.append(relVeloChange[gen1_Edges[i]])
    
            for i in range(len(gen2_Edges)):
                 self._gen2_absFlowChange.append(absFlowChange[gen2_Edges[i]])
                 self._gen2_relFlowChange.append(relFlowChange[gen2_Edges[i]])
                 self._gen2_absVeloChange.append(absVeloChange[gen2_Edges[i]])
                 self._gen2_relVeloChange.append(relVeloChange[gen2_Edges[i]])
    
            for i in range(len(rem_Edges)):
                 self._gen3_absFlowChange.append(absFlowChange[rem_Edges[i]])
                 self._gen3_relFlowChange.append(relFlowChange[rem_Edges[i]])
                 self._gen3_absVeloChange.append(absVeloChange[rem_Edges[i]])
                 self._gen3_relVeloChange.append(relVeloChange[rem_Edges[i]])
    ...
    def _update_eff_resistance_and_LS(self, newGraph=None, vertex=None,
        ...
        if vertex is None:
            ...
            vertexList = np.concatenate([[vertex],
...
#                    G.es[eiIn]['path'][-1]= 0
		...
		#Move 'tRBC' value with RBC to new edge
		    ...
		    if G.es[eiIn]['sign'] == 1:		
		        e['tRBC']=np.concatenate((G.es[eiIn]['tRBC'][-1::],e['tRBC']))
                        #e['path']=np.concatenate((G.es[eiIn]['path'][-1::],e['path']))
		    else:
                        e['tRBC']=np.concatenate((G.es[eiIn]['tRBC'][0:1],e['tRBC']))
                        #e['path']=np.concatenate((G.es[eiIn]['path'][0:1],e['path']))
            else:
		#Move 'tRBC' value with RBC to new edge
		    ...
		    if G.es[eiIn]['sign'] == 1:
                        ...
                        #e['path']=np.concatenate((e['path'],G.es[eiIn]['path'][-1::]))
		    else:
		        #e['path']=np.concatenate((e['path'],G.es[eiIn]['path'][0:1]))	
		...
		if len(e['tRBC']) > 0 and (len(e['rRBC']) == len(e['tRBC'])):
			...
			#transitTimeDict['path'].append(e['path'][0])
                        transitTimeDict['time'].append(time)
                    e['tRBC']=e['tRBC'][1:]
                    #e['path']=e['path'][1:]
                e['rRBC'] = e['rRBC'][1:]
	...
	timelistAvg = self._timelistAvg
        ...
        vgm.write_pkl(self._gen0_absFlowChange,'Gen0_absFlow.pkl')
        vgm.write_pkl(self._gen1_absFlowChange,'Gen1_absFlow.pkl')
        vgm.write_pkl(self._gen2_absFlowChange,'Gen2_absFlow.pkl')
        vgm.write_pkl(self._gen3_absFlowChange,'Gen3_absFlow.pkl')
        vgm.write_pkl(self._gen0_relFlowChange,'Gen0_relFlow.pkl')
        vgm.write_pkl(self._gen1_relFlowChange,'Gen1_relFlow.pkl')
        vgm.write_pkl(self._gen2_relFlowChange,'Gen2_relFlow.pkl')
        vgm.write_pkl(self._gen3_relFlowChange,'Gen3_relFlow.pkl')
        vgm.write_pkl(self._gen0_absVeloChange,'Gen0_absVelo.pkl')
        vgm.write_pkl(self._gen1_absVeloChange,'Gen1_absVelo.pkl')
        vgm.write_pkl(self._gen2_absVeloChange,'Gen2_absVelo.pkl')
        vgm.write_pkl(self._gen3_absVeloChange,'Gen3_absVelo.pkl')
        vgm.write_pkl(self._gen0_relVeloChange,'Gen0_relVelo.pkl')
        vgm.write_pkl(self._gen1_relVeloChange,'Gen1_relVelo.pkl')
        vgm.write_pkl(self._gen2_relVeloChange,'Gen2_relVelo.pkl')
        vgm.write_pkl(self._gen3_relVeloChange,'Gen3_relVelo.pkl')
