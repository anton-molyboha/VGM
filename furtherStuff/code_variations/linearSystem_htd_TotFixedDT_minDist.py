...
__all__ = ['LinearSystemHtdTotFixedDTsmallMinDist']
...
class LinearSystemHtdTotFixedDTsmallMinDist(object):
    """The discrete model is extended such, that a fixed timestep is given. Hence,
    more than one RBC will move per time step.
    There is a second edge propertie e['minDist2'] which defines how close RBCs
    can get to one and another. The minimum distance between RBCs is reduced.
    """
    #@profile
	...
	if init:
        ...
        G.es['minDist2'] = np.array(G.es['minDist'])*np.array([0.1]*G.ecount())
...
#                      np.array(G.es['effResistance'])
	...
	#No flow Edges are not considered for the propagation of RBCs
        ...
        #-------------------------------------------------------------------------------------------
			...
			#distToFirst = distance to first vertex in vessel
                        ...
                        posNoBifEvents=int(np.floor(distToFirst/oe['minDist2']))
			...
			#and that RBCs are only 'half' in the vessel 
			    ...
			    #BUT due to different velocity factors RBCs cann "ran into each other" at connecting bifurcations
                            ...
                            for i in range(overshootsNo-1):
                                index=-1*(i+1)
                                if position[index]-position[index-1] < oe['minDist2']:
                                    position[index-1]=position[index]-oe['minDist2']
                            ...
                            if len(oe['rRBC']) > 0:
                                if oe['sign'] == 1 and position[-1] > oe['rRBC'][0]-oe['minDist2']:
                                    posLead=position[-1]
                                    position = np.array(position)-np.array([posLead-(oe['rRBC'][0]-oe['minDist2'])]*len(position))
                                    for i in range(overshootsNo):
                                        if position[i] < 0:
                                            overshootsNoReduce2 += 1
                                        else:
                                            break
                                elif oe['sign'] == -1 and position[-1] > oe['length']-oe['rRBC'][-1]-oe['minDist2']:
                                    posLead=position[-1]
                                    position = np.array(position)-np.array([posLead-(oe['length']-oe['rRBC'][-1]-oe['minDist2'])]*len(position))
                                    for i in range(overshootsNo):
                                        if position[i] < 0:
                                            overshootsNoReduce2 += 1
                                        else:
                        ...
                        for i in range(noStuckRBCs):
                            ...
                            e['rRBC'][index]=e['length']-i*e['minDist2'] if sign == 1.0 else 0+i*e['minDist2']
                        #Recheck if the distance between the newly introduces RBCs is still big enough 
                        if len(e['rRBC']) > 1:
                            moved = 0
                            count = 0
                            if sign == 1.0:
                                for i in range(-1,-1*(len(e['rRBC'])),-1):
                                    index=i-1
                                    if e['rRBC'][i] < e['rRBC'][index] or abs(e['rRBC'][i]-e['rRBC'][index]) < e['minDist2']:
                                        e['rRBC'][index]=e['rRBC'][i]-e['minDist2']
                                        moved = 1
                                    else:
                                        moved = 0
                                    count += 1
          ...
          #-------------------------------------------------------------------------------------------
                        ...
                        posNoBifEventsPref=np.floor(distToFirst/oe['minDist2'])
                        ...
                        posNoBifEventsPref2=np.floor(distToFirst2/oe2['minDist2'])
                        ...
                        if len(outEdges) > 2:
                            posNoBifEventsPref3=np.floor(distToFirst3/oe3['minDist2'])
                                            ...
                                            if posNoBifEventsPref2 > countPref2 and pref2Full == 0:
                                                        ...
                                                        if len(outEdges)>2:
                                                            ...
                                                            if posNoBifEventsPref3 > countPref3 and pref3Full == 0:
                                                                ...
                                                                if positionPref3 != []: 
                                                                    ...
                                                                    if dist3 < oe3['minDist2']:
                                                                        space1 =  positionPref1[-1] if oe['sign'] == 1.0 \
                                                                            else oe['length']-positionPref1[-1]
                                                                        if np.floor(space1/oe['minDist2']) >= 1:
                                                                            timeBlocked1=(oe['minDist2']-dist1)/oe['v']
                                                                        else:
                                                                            timeBlocked1=None
                                                                            pref1Full=1
                                                                        space2 =  positionPref2[-1] if oe2['sign'] == 1.0 \
                                                                            else oe2['length']-positionPref2[-1]
                                                                        if np.floor(space2/oe2['minDist2']) >= 1:
                                                                            timeBlocked2=(oe2['minDist2']-dist2)/oe2['v']
                                                                        else:
                                                                            timeBlocked2=None
                                                                            pref2Full=1
                                                                        space3 =  positionPref3[-1] if oe3['sign'] == 1.0 \
                                                                            else oe3['length']-positionPref3[-1]
                                                                        if np.floor(space3/oe3['minDist2']) >= 1:
                                                                            timeBlocked3=(oe3['minDist2']-dist3)/oe3['v']
                                                                        else:
                                                                            timeBlocked3=None
                                                                            pref3Full=1
                                                                        ...
                                                                        if newOutEdge == 1:
                                                                            if oe['sign'] == 1.0:
                                                                                position1[index1]=positionPref1[-1]-oe['minDist2']
                                                                                if position1[index1] > 0:
                                                                                    ...
                                                                                    print(np.floor(space1/oe['minDist2']))
                                                                                    print(timeBlocked1)
                                                                                    print(pref1Full)
                                                                            else:
                                                                                position1[index1]=positionPref1[-1]+oe['minDist2']
                                                                            ...
                                                                            if oe2['sign'] == 1.0:
                                                                                position2[index2]=positionPref2[-1]-oe2['minDist2']
                                                                                if position2[index2] > 0:
                                                                                    ...
                                                                                    print(np.floor(space2/oe2['minDist2']))
                                                                                    print(timeBlocked2)
                                                                                    print(pref2Full)
                                                                            else:
                                                                                position2[index2]=positionPref2[-1]+oe2['minDist2']
                                                                            ...
                                                                            if oe3['sign'] == 1.0:
                                                                                position3[index3]=positionPref3[-1]-oe3['minDist2']
                                                                                if position3[index3] > 0:
                                                                                    ...
                                                                                    print(np.floor(space3/oe3['minDist2']))
                                                                                    print(timeBlocked3)
                                                                                    print(pref3Full)
                                                                            else:
                                                                                position3[index3]=positionPref3[-1]+oe3['minDist2']
                                                                ...
                                                                #No RBCs have been put in outEPref3 so far
                                                                    ...
                                                                    if oe3['sign'] == 1.0:
                                                                        ...
                                                                        if len(oe3['rRBC']) > 0:
                                                                            if oe3['rRBC'][0]-position3[index3] < oe3['minDist2']:
                                                                                position3[index3]=oe3['rRBC'][0]-oe3['minDist2']
                                                                        ...
                                                                        #Check that there is no overtaking of RBCs
                                                                        if len(oe3['rRBC']) > 0:
                                                                            if position3[index3]-oe3['rRBC'][-1] < oe3['minDist2']:
                                                                              position3[index3]=oe3['rRBC'][-1]+oe3['minDist2']
                                                                        else:
                                                                            #Avoid overshooting whole vessels
                                                                            if position3[index3] < 0:
                                                                                position3[index3]=0
                                                ...
                                                #no RBCs have been put in outEPref2 so far
                                                    ...
                                                    if oe2['sign'] == 1.0:
                                                        ...
                                                        #Check that there is no overtaking of RBCs
                                                        if len(oe2['rRBC']) > 0:
                                                            if oe2['rRBC'][0]-position2[index2] < oe2['minDist2']:
                                                                position2[index2]=oe2['rRBC'][0]-oe2['minDist2']
                                                        ...
                                                        if len(oe2['rRBC']) > 0:
                                                            if position2[index2]-oe2['rRBC'][-1] < oe2['minDist2']:
                                                                position2[index2]=oe2['rRBC'][-1]+oe2['minDist2']
                                                        else:
                                                            if position2[index2] < 0:
                                                                position2[index2]=0
                                                    positionPref2.append(position2[index2])
					    ...
					    #Check if there is a third outEdge
                                    ...
                                    #There are not yet any new RBCs in outEdgePref
                                        ...
                                        if oe['sign'] == 1.0:
                                            ...
                                            #Check that there is no overtaking of RBCs
                                            if len(oe['rRBC']) > 0:
                                                if oe['rRBC'][0]-position1[index1] < oe['minDist2']:
                                                    position1[index1]=oe['rRBC'][0]-oe['minDist2']
                                            ...
                                            if len(oe['rRBC']) > 0:
                                                if position1[index1]-oe['rRBC'][-1] <oe['minDist2']:
                                                    position1[index1]=oe['rRBC'][-1]+oe['minDist2']
                                            else:
                                                if position1[index1] < 0:
                                                    position1[index1]=0
                                        positionPref1.append(position1[index1])
    ...
    #-------------------------------------------------------------------------------------------
                        ...
                        posNoBifEvents=int(np.floor(distToFirst/G.es[outE]['minDist2']))
                        ...
                        if posNoBifEvents > 0:
                            ...
                            if len(G.es[outE]['rRBC']) > 0:
                                if G.es[outE]['sign'] == 1.0:
                                    if position[-1] > G.es[outE]['rRBC'][0]-G.es[outE]['minDist2']:
                                        position[-1]=G.es[outE]['rRBC'][0]-G.es[outE]['minDist2']
                                else:
                                    if G.es[outE]['length']-position[-1] < G.es[outE]['rRBC'][-1]+G.es[outE]['minDist2']:
                                        position[-1]=G.es[outE]['length']-(G.es[outE]['rRBC'][-1]+G.es[outE]['minDist2'])
                            else:
                            ...
                            for i in range(-1,-1*(count1+count2+count3),-1):
                                if position[i]-position[i-1] < G.es[outE]['minDist2'] or \
                                    position[i-1] > position[i]:
                                    position[i-1]=position[i]-G.es[outE]['minDist2']
                        ...
                        for i in range(noStuckRBCs2):
                            ...
                            e2['rRBC'][index]=e2['length']-i*e2['minDist2'] if sign2 == 1.0 else 0+i*e2['minDist2']
                        #Recheck if the distance between the newly introduces RBCs is still big enough 
                        if len(e2['rRBC']) >1:
                            moved = 0
                            count = 0
                            if sign2 == 1.0:
                                for i in range(-1,-1*(len(e2['rRBC'])),-1):
                                    index=i-1
                                    if e2['rRBC'][i] < e2['rRBC'][index] or abs(e2['rRBC'][i]-e2['rRBC'][index]) < e2['minDist2']:
                                        e2['rRBC'][index]=e2['rRBC'][i]-e2['minDist2']
                                        moved = 1
                                    else:
                                        moved = 0
                                    count += 1
                        ...
                        if len(inflowEdges) > 2:
                            ...
                            for i in range(noStuckRBCs3):
                                ...
                                e3['rRBC'][index]=e3['length']-i*e3['minDist2'] if sign3 == 1.0 else 0+i*e3['minDist2']
                            #Recheck if the distance between the newly introduces RBCs is still big enough 
                            if len(e3['rRBC']) >1:
                                moved = 0
                                count = 0
                                if sign3 == 1.0:
                                    for i in range(-1,-1*(len(e3['rRBC'])),-1):
                                        index=i-1
                                        if e3['rRBC'][i] < e3['rRBC'][index] or abs(e3['rRBC'][i]-e3['rRBC'][index]) < e3['minDist2']:
                                            e3['rRBC'][index]=e3['rRBC'][i]-e3['minDist2']
                                            moved = 1
                                        else:
                                            moved = 0
                                        count += 1
